...
import { FolderGit2, Search, Bell, Plus, ChevronDown, Star, GitFork, Eye, Play, History, Code, MessageSquare, Layout, Activity, Shield, Book, CircleDot, GitPullRequest, Settings, Terminal, Bot, X } from 'lucide-react';
...
export default function RepoPage() {
  ...
  const [geminiOutput, setGeminiOutput] = useState("Gemini AI loaded. Ask questions about the active file.\n");
  ...
  const fetchRepo = async () => {
    try {
      ...
      // If we don't have an active file but we have files, select the first one (or README)
      if (data.files && data.files.length > 0 && !activeFile) {
        ...
        handleFileSelect(readme || data.files[0]);
         ...
         // update active file content if it was modified elsewhere
  ...
  const handleAiAction = async (action: 'explain' | 'review') => {
    ...
    setIsAiLoading(true);
    setGeminiOutput(prev => prev + `\n> Requesting ${action} for ${activeFile.path}...\n`);
      ...
      <div className="bg-[#0d1117] border-b border-[#30363d] pt-4 px-6 shrink-0 flex flex-col gap-4">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3 text-sm">
             <button className="bg-[#21262d] border border-[#30363d] text-[#c9d1d9] px-3 py-1 rounded-md hover:bg-[#30363d] hover:text-white transition-colors flex items-center gap-1.5">
                <Shield className="w-4 h-4 text-indigo-400" /> Intent Mode
             </button>
                ...
                <>
                  ...
                  <div className="h-[50px] border-b border-[#30363d] bg-[#0d1117] flex items-center justify-between px-4 shrink-0">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => handleSaveFile()} className="bg-[#6366f1] hover:bg-[#4f46e5] text-white text-xs font-bold px-4 py-1.5 rounded-md flex items-center gap-1.5 transition-colors">
                         PUSH
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="h-64 border-t border-[#30363d] bg-[#0d1117] flex flex-col shrink-0">
                     <div className="flex items-center justify-between border-b border-[#30363d] px-4 h-10 bg-[#0d1117]">
                        ...
                        <button 
                          onClick={() => bottomPanelTab === 'terminal' ? setTerminalOutput('') : setGeminiOutput('')}
                     ...
                     <div className="flex-1 overflow-y-auto p-4 font-mono text-sm">
                        {bottomPanelTab === 'terminal' ? (
                          <pre className="text-[#c9d1d9] whitespace-pre-wrap">{terminalOutput}</pre>
                        ) : (
                          <div className="flex flex-col h-full">
                            <pre className="text-[#c9d1d9] whitespace-pre-wrap flex-1 overflow-y-auto">{geminiOutput}</pre>
                            {isAiLoading && <div className="text-[#58a6ff] animate-pulse mt-2">Generating...</div>}
                            <div className="flex gap-2 mt-2">
                               <button onClick={() => handleAiAction('explain')} disabled={isAiLoading} className="bg-[#21262d] text-white text-xs px-3 py-1 rounded border border-[#30363d] hover:bg-[#30363d] disabled:opacity-50">Explain Code</button>
                               <button onClick={() => handleAiAction('review')} disabled={isAiLoading} className="bg-[#21262d] text-white text-xs px-3 py-1 rounded border border-[#30363d] hover:bg-[#30363d] disabled:opacity-50">Review Code</button>
                            </div>
                          </div>
                        )}
                     </div>
